...
class SDXLPanoramaGenerator:
    def __init__(self, model_id="stabilityai/stable-diffusion-xl-base-1.0", device="cuda"):
        ...
        # 2. SWITCH TO DDIM (Stateless Scheduler)
        # Allows stepping multiple times per timestep without breaking internal counters
        self.pipe.scheduler = DDIMScheduler.from_config(
            self.pipe.scheduler.config, 
            steps_offset=1
        )
        
        # 3. Force UNet to Float16 (Speed)
        self.pipe.unet.to(dtype=torch.float16)
        
        # 4. Force VAE to Float32 (Prevents Black/NaN pixels)
        self.pipe.vae.to(dtype=torch.float32)
        self.pipe.enable_vae_tiling()

    @torch.no_grad()
    def get_views(self, panorama_latents, window_size=128, stride=64):
        """ Cropping Logic with Circular Wrap-Around """
        views = []
    ...
    def merge_latents(self, views, latent_shape, window_size=128, stride=64):
        """ Merges the PREDICTED latents (MultiDiffusion) """
    ...
    @torch.no_grad()
    def generate(self, prompt, height=1024, width=4096, steps=50, guidance_scale=7.5):
        ...
        # 3. Time IDs (Coordinate conditioning fixed to 1024x1024)
